...
USERS_PER_PAGE = 8   # пагинация списка пользователей
...
def init_db():
    with get_conn() as conn:
        ...
        _safe_add_columns(conn, "funnel_content", [
            ("btn_text", 'TEXT DEFAULT ""'),
            ("btn_url",  'TEXT DEFAULT ""'),
        ])
...
#  ПОЛЬЗОВАТЕЛИ
# ══════════════════════════════════════════════════════════════
def upsert_user(tg_id: int, tag: str,
                username: str = None, full_name: str = None):
    with get_conn() as conn:
        now = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        conn.execute('''
            INSERT INTO users (tg_id, username, full_name, tag, join_date,
                               funnel_stopped, last_warmup_sent)
            VALUES (?, ?, ?, ?, ?, 0, 0)
...
def get_user_by_phone(phone: str):
    with get_conn() as conn:
        return conn.execute(
            'SELECT * FROM users WHERE phone LIKE ?', (f'%{phone}%',)
        ).fetchone()
    ...
    with get_conn() as conn:
        ...
        conn.execute('''
            ...
            SET is_paid = 1, paid_date = ?,
                funnel_stopped = 1, post_webinar_active = 0
            WHERE tg_id = ?
        ''', (now, tg_id))


def set_last_buy_intent(tg_id: int, intent: str):
    """intent: course_self | course_pro | razbor | vip | diag"""
    with get_conn() as conn:
...
def mark_buy_click(tg_id: int) -> bool:
    with get_conn() as conn:
        ...
        if user and not user['clicked_buy']:
            now = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            conn.execute('''
                UPDATE users SET clicked_buy = 1, click_buy_time = ?
                WHERE tg_id = ?
            ''', (now, tg_id))
            return True
    return False
...
def get_razbor_reminder_candidates():
    with get_conn() as conn:
        return conn.execute('''
            SELECT * FROM users
            WHERE razbor_pay_clicked = 1 AND razbor_paid = 0 AND is_paid = 0
        ''').fetchall()
...
def get_vip_reminder_candidates():
    with get_conn() as conn:
        return conn.execute('''
            SELECT * FROM users
            WHERE vip_clicked = 1 AND vip_paid = 0 AND is_paid = 0
        ''').fetchall()
...
def get_diag_reminders():
    with get_conn() as conn:
        return conn.execute('SELECT * FROM users WHERE diag_clicked=1 AND diag_paid=0 AND is_paid=0').fetchall()
...
def start_post_webinar(tg_id: int):
    with get_conn() as conn:
        now = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        conn.execute('''
            UPDATE users SET post_webinar_active=1, post_webinar_step=0,
                             post_webinar_start=? WHERE tg_id=?
        ''', (now, tg_id))
...
def increment_post_webinar_step(tg_id: int):
    with get_conn() as conn:
        conn.execute(
            'UPDATE users SET post_webinar_step = post_webinar_step + 1 WHERE tg_id = ?',
            (tg_id,)
        )
...
def get_warmup_users():
    with get_conn() as conn:
        return conn.execute('''
            SELECT * FROM users
            WHERE is_paid = 0 AND funnel_stopped = 0
              AND tag IN ('отёки','подтяжка','обучение')
        ''').fetchall()
...
def get_post_webinar_users():
    with get_conn() as conn:
        return conn.execute('''
            SELECT * FROM users
            WHERE is_paid = 0 AND post_webinar_active = 1 AND funnel_stopped = 0
        ''').fetchall()
...
def get_webinar_registered():
    with get_conn() as conn:
        return conn.execute(
            'SELECT * FROM users WHERE webinar_registered = 1 AND is_paid = 0'
        ).fetchall()


def get_users_by_tag(tag: str):
    with get_conn() as conn:
        return conn.execute('SELECT * FROM users WHERE tag = ?', (tag,)).fetchall()
...
def get_users_page(offset: int):
    """Возвращает срез для пагинации и общее кол-во."""
    with get_conn() as conn:
...
def set_webinar_reminder_sent(kind: str):
    ...
    with get_conn() as conn:
        conn.execute(f'UPDATE webinar_settings SET {col} = 1 WHERE id = 1')
...
def get_stats() -> dict:
    with get_conn() as conn:
        ...
        post_web     = conn.execute('SELECT COUNT(*) FROM users WHERE post_webinar_active=1').fetchone()[0]
        ...
        by_tag = {
            ...
            for row in conn.execute(
                'SELECT tag, COUNT(*) as cnt FROM users GROUP BY tag'
            ).fetchall()
        }
    return dict(total=total, paid=paid, webinar_registered=web_reg,
                webinar_attended=web_att, clicked_buy=clicked_buy,
                post_webinar=post_web, razbor_paid=razbor_paid,
                vip_paid=vip_paid, diag_paid=diag_paid, by_tag=by_tag)


# ─── Контент воронки (переопределения из админки) ───────────
def get_funnel_content(tag: str, day: int):
    """Возвращает Row или None если нет переопределения."""
    with get_conn() as conn:
        return conn.execute(
            'SELECT * FROM funnel_content WHERE id = ?',
            (f"{tag}_{day}",)
        ).fetchone()


def set_funnel_content(tag: str, day: int, text: str,
                       media_file_id: str = "", media_type: str = "",
