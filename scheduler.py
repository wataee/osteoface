...
from config import MONECLE_API_URL, MONECLE_API_ID, MONECLE_API_KEY, ADMIN_GROUP_ID, TIMEZONE
from content import *
...
def _warmup_keyboard(day: int, tag: str, tg_id: int,
                     has_webinar: bool):
    if day == 1:
        return kb_warmup_day1(tag)
    if day == 2:
        return kb_warmup_day2(tag)
    if day == 3:
...
# ══════════════════════════════════════════════════════════════
#  JOB 5 — ДОЖИМ ДИАГНОСТИКИ ПРЕДНАЗНАЧЕНИЯ
# ══════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════
#  JOB 6 — ПРОВЕРКА ОПЛАТ ЧЕРЕЗ MONECLE API
# ══════════════════════════════════════════════════════════════
        ...
        if user["razbor_pay_click_time"] and not user["razbor_paid"]:
            try:
                ...
                if step == 0 and cond_6h:
                    ...
                    await bot.send_message(tg_id, RAZBOR_REMIND_48H, reply_markup=kb_razbor_personal_pay(tg_id))
...
#  НАСТРОЙКА SCHEDULER
# ══════════════════════════════════════════════════════════════
def setup_scheduler(bot: Bot, is_test: bool = False):
    """Настройка всех джобов шедулера"""
